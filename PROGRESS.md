...
- [`include/ftj_engine.hpp`](file:///D:/FTJ-SSD-Sim/include/ftj_engine.hpp): Header defining the `FTJController` and `LatencyInjector` classes with TSC-based low-overhead nanosecond latency injection.
- [`src/ftj_engine.cpp`](file:///D:/FTJ-SSD-Sim/src/ftj_engine.cpp): Implementation of the physical FTJ memory simulation and high-precision timer calibration on Windows.
- [`src/main.cpp`](file:///D:/FTJ-SSD-Sim/src/main.cpp): Windows CLI entry point executing synthetic workloads and comparing simulated FTJ against 3D NAND.
