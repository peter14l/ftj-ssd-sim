...
namespace ftj {
    FTJController* g_controller = nullptr;
}
#include <cassert>
#include <map>
    ...
    PULONG PBytesTransferred)
{
    ...
    if (wcscmp(FileNode->FileName, L"\\ftj_disk.bin") == 0) {
        if (Offset >= FileNode->FileInfo.FileSize)
            return STATUS_END_OF_FILE;
        EndOffset = Offset + Length;
        if (EndOffset > FileNode->FileInfo.FileSize)
            EndOffset = FileNode->FileInfo.FileSize;
        if (ftj::g_controller && ftj::g_controller->Read(Offset, Buffer, (ULONG)(EndOffset - Offset))) {
    ...
    PULONG PBytesTransferred, FSP_FSCTL_FILE_INFO *FileInfo)
{
    ...
    if (wcscmp(FileNode->FileName, L"\\ftj_disk.bin") == 0) {
        ...
        if (ftj::g_controller && ftj::g_controller->Write(Offset, Buffer, (ULONG)(EndOffset - Offset))) {
            *PBytesTransferred = (ULONG)(EndOffset - Offset);
            if (FileInfo) {
                FileNode->FileInfo.LastWriteTime = FileNode->FileInfo.ChangeTime = MemfsGetSystemTime();
                memcpy(FileInfo, &FileNode->FileInfo, sizeof(*FileInfo));
            }
            return STATUS_SUCCESS;
        }
        return STATUS_INVALID_PARAMETER;
    ...
    MEMFS **PMemfs)
{
    ...
    *PMemfs = 0;
    ...
    // Initialize latency injector once
    static bool calibrated = false;
    if (!calibrated) {
        ftj::LatencyInjector::Calibrate();
        calibrated = true;
    }
    // Instantiate 128 MB simulator space controller if not already done
    if (!ftj::g_controller) {
        ftj::g_controller = new ftj::FTJController(128 * 1024 * 1024, 8);
    }

    wcscpy_s(VolumeParams.FileSystemName, sizeof VolumeParams.FileSystemName / sizeof(WCHAR),
        0 != FileSystemName ? FileSystemName : L"FTJFS");
    ...
    Memfs->VolumeLabelLength = sizeof L"FTJFS" - sizeof(WCHAR);
    memcpy(Memfs->VolumeLabel, L"FTJFS", Memfs->VolumeLabelLength);
    ...
    // Create pre-defined virtual disk file for the simulator
    MEMFS_FILE_NODE* DiskNode = nullptr;
    Result = MemfsFileNodeCreate(L"\\ftj_disk.bin", &DiskNode);
    if (NT_SUCCESS(Result)) {
        DiskNode->FileInfo.FileAttributes = FILE_ATTRIBUTE_NORMAL;
        DiskNode->FileInfo.FileSize = ftj::g_controller->GetCapacity();
        DiskNode->FileInfo.AllocationSize = ftj::g_controller->GetCapacity();
        
        // Copy security descriptor from root for simplicity
        DiskNode->FileSecurity = malloc(RootSecuritySize);
        if (DiskNode->FileSecurity) {
            DiskNode->FileSecuritySize = RootSecuritySize;
            memcpy(DiskNode->FileSecurity, RootSecurity, RootSecuritySize);
        }
        
        // Insert into FileNodeMap
        Result = MemfsFileNodeMapInsert(Memfs->FileNodeMap, DiskNode, &Inserted);
        if (!NT_SUCCESS(Result)) {
            MemfsFileNodeDelete(DiskNode);
        }
    }
